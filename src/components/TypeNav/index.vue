...
export default {
  ...
  mounted() {
    //派发action，为了避免每次跳转路由时多次发送请求，消耗性能，所以我们把派发action的代码放到App组件当中，因为它的mounted只执行一次,不能放在main.js入口文件内，因为它不是组件，没有$store。
    // this.$store.dispatch("getCategory");
    //路由切换的时候,路由组件会被销毁重建【子组件不也是】
    // 判断当前组件是不是search，如果不是home，则隐藏
    if (this.$route.path != "/home") {
      this.show = false;
    }
  },
