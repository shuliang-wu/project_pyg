...
export default {
  ...
  computed: {
    ...
    spuSaleAttrValueList() {
      return this.spuSaleAttrList.spuSaleAttrValueList || [];
    },
  },
  methods: {
    // 点击切换高亮效果
    changeActive(attrValue, arr) {
      // console.log(attrValue,arr);
      // 拿到当前点击的项和装它的数组，遍历数组，排他思想，全部清空isChecked属性，只让当前点击的项高亮
      arr.forEach((item) => {
        item.isChecked = 0;
      });
    ...
    // 判断用户输入商品数量
    changeNum(event) {
      // 让用户输入的结果*1，如果是非数字，结果为NaN ，或者用户输入负数，我们都让结果变为1
      let result = event.target.value * 1;
      if (isNaN(result) || result < 1) {
        this.skuNum = 1;
      } else {
        // 如果用户输入的是合法的数字，我们还需要取整
        this.skuNum = parseInt(result);
      }
    },
    // 点击加入购物车
    async addShopCart() {
      // 这里点击加入购物车，不仅仅是跳转路由，还需要将用户选择的参数发送给服务器存储，服务器存储成功后，才能进行路由的跳转，否则提示用户失败
      // 当前这里派发一个action，向服务器发送请求，判断加入购物车是成功了还是失败了，进行相应的操作
      // 说白了：调用仓库中的addShopCart，这个方法加上async，返回的一定是一个promise
      // 要么成功、要么失败
      try {
        // 成功
        ...
        // 如果成功了，就跳转路由到加入购物车成功页面
        // 在进行路由跳转的时候，将产品的信息带给下一级的路由组件
        // 下面这种跳转路由+传参的方式是可行的，但是有一个问题就是地址栏会变得很不美观，且乱，所以我们只携带产品的个数参数
        // this.$router.push({name:"addcartsuccess",query:{skuInfo:this.$route.skuInfo,skuNum:this.skuNum}});
        // 简单的参数【skuNum】我们可以使用query参数传递过去，复杂的数据【skuInfo】我们可以使用会话存储的方式存储，然后需要数据的组件直接读取即可；
        // 注意：本地存储|会话存储 传递的只能是字符串！！！不能传递对象等数据，需要先转换为字符串存储，然后再转换成对象读取
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))//第一个参数是名字，第二个参数是存入的数据
